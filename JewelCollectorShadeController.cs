...
public class JewelCollectorShadeController : MonoBehaviour {
	...
	void FixedUpdate () {
		if (darkenShade) {
			if (instantiateOwlie) {
				...
				instantiatedSpeechBubble = (GameObject)Instantiate (speechBubble, new Vector3 (-10, 3.02f, -101f), Quaternion.identity);
				instantiatedSpeechBubble.GetComponent<LevelFailedScreenSlider> ().SetTargetX (-.94f);
				currentOwlie = (GameObject)Instantiate (owlie, new Vector3 (-10, 1.23f, -100.5f), Quaternion.identity);
