...
public class ShadeTouchHandler : MonoBehaviour {
	...
	void Update () {
		...
		if (lightenShade) {
			...
			oldColor = spriteRenderer.color;
			if (oldColor.a <= 0) {
				GameObject.Find ("Screen Handlers").GetComponent<PurchaseLivesScreen> ().SetShadeInstantiated (false);
				Destroy (gameObject);
			}
		}
	}
	...
	void CheckTouch2 (Vector3 pos) {
		...
		if (hit2 != null && hit2.gameObject.name != "Shade(Clone)")
	...
	void CheckTouch3 (Vector3 pos) {
		...
		if (!wanderedAwayFromShade && hit != null && hit.gameObject.name == "Shade(Clone)" && hit2 != null && hit.gameObject.name == "Shade(Clone)" && hit3 != null && hit3.gameObject.name == "Shade(Clone)") {
			...
			else if (GameObject.Find ("Purchase Lives Button(Clone)") != null) {
