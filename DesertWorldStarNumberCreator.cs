...
public class DesertWorldStarNumberCreator : MonoBehaviour {
	...
	void Awake () {
		...
		if (totalStars < 180) {
			...
			instantiatedPadlock.transform.position = new Vector3 (25.762f, 1.084f, -5);
			instantiatedPadlock.transform.parent = transform;
		} else if (totalStars >= 180 || PlayerPrefs.HasKey ("finishedAllLaunchPadLevels")){
			touchOn = true;
	...
	public void FinishedAllLaunchPadLevels () {
		if (!PlayerPrefs.HasKey ("finishedAllLaunchPadLevels")) 
			PlayerPrefs.SetString ("finishedAllLaunchPadLevels", "true");
	}
