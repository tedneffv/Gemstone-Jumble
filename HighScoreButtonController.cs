...
public class HighScoreButtonController : MonoBehaviour {
	...
	public GameObject pressedHighScoreButton;
	GameObject instantiatedHighScoreButton;
	...
	void CheckTouch (Vector3 pos) {
		...
		if (hit != null && hit.gameObject == gameObject) {
			instantiatedHighScoreButton = (GameObject)Instantiate (pressedHighScoreButton, new Vector3 (transform.position.x, transform.position.y, transform.position.z - .01f), Quaternion.identity);
			instantiatedHighScoreButton.transform.parent = transform;
		}
	}
	...
	void CheckTouch2 (Vector3 pos) {
		...
		if (hit2 == null || (hit2 != null && hit2.gameObject != gameObject)) {
			if (instantiatedHighScoreButton != null) 
				Destroy (instantiatedHighScoreButton);
		}
	}
	...
	void CheckTouch3 (Vector3 pos) {
		...
		if (hit3 != null && hit3.gameObject == gameObject) {
			if (instantiatedHighScoreButton != null) 
				Debug.Log ("High Score Button Presed");
		}
