...
public class LaunchpadLevelThirtyCreator : MonoBehaviour {
	...
	GameObject GetCorrectBomb (int bombNumber, Vector3 position) {
		switch (bombNumber) {
		case 100: return GetBomb (position, bombNumber75);
		}
		return null;
	}
	...
	GameObject GetJewels (int jewelNumber, Vector3 position) {
		GameObject temp;
		switch (jewelNumber) {
		case 0: temp = GetSlugBomb (position, bombNumber75, slugStage4);
		...
		case 4: temp = GetSlugBomb (position, bombNumber75, slugStage4);
			SlugListManager.AddToSlugList (temp);
			return temp;
		...
		case 8: temp = GetSlugBomb (position, bombNumber75, slugStage4);
			SlugListManager.AddToSlugList (temp);
			return temp;
