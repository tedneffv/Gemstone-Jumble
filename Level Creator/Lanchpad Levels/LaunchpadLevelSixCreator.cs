...
public class LaunchpadLevelSixCreator : MonoBehaviour {
	...
	GameObject GetCorrectBomb (int bombNumber, Vector3 position) {
		switch (bombNumber) {
		case 100: return GetBomb (position, bombNumber20);
		}
		return null;
	}
	...
	GameObject GetJewels (int jewelNumber, Vector3 position) {
		...
		case 8: temp = GetSlugBomb (position, bombNumber20, slugStage4);
		...
		case 72: temp = GetSlugBomb (position, bombNumber20, slugStage4);
			SlugListManager.AddToSlugList (temp);
			return temp;
